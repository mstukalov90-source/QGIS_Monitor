...
4. **Границы районов**
5. **Объекты ДГИ и ООЗТ** (по умолчанию скрыта) — слой `oozt` (ООЗТ) и три слоя `dgi` с `sql_filter`
...
4. Символика камер; **Объекты ДГИ и ООЗТ** — ООЗТ светло-зелёный со штриховкой, 3 слоя собственности; «Фото разрытий» — точки с красной обводкой; ордера/уведомления — по таблицам `_points` / `_lines` / `_polygons`.
